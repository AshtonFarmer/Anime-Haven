(()=>{
  ...
  function migrateLibrary(){
    ...
    profile.items.forEach(item=>{
      ...
      if(!item.progressNote||/season|episode|haven.t started|movie|ova/i.test(item.progressNote)){
  ...
  async function searchMedia(search){
    const data=await aniListRequest(`query($search:String){Page(page:1,perPage:12){media(search:$search,type:ANIME,sort:SEARCH_MATCH){${MEDIA_FIELDS}}}}`,{search});
    return data?.Page?.media||[];
  }
  async function mediaById(id){
    const data=await aniListRequest(`query($id:Int){Media(id:$id,type:ANIME){${MEDIA_FIELDS}}}`,{id:Number(id)});
    return data?.Media;
  }

  function titleOf(media){return media?.title?.english||media?.title?.romaji||media?.title?.native||'Untitled Anime'}
  function dateKey(date){return date?.year?date.year*10000+(date.month||1)*100+(date.day||1):99999999}
  function isMovie(media){return String(media?.format||'').includes('MOVIE')}
  ...
  function addMedia(media,startNow){
    ...
    const relations=(media.relations?.edges||[]).filter(edge=>['PREQUEL','SEQUEL','PARENT','SIDE_STORY','ALTERNATIVE'].includes(edge.relationType)).map(edge=>({
      type:edge.relationType,id:edge.node.id,title:titleOf(edge.node),date:dateKey(edge.node.startDate)
    ...
    state.updatedAt=now();writeState(state);
  ...
  function createModal(){
    ...
    dialog.querySelector('#addAnimeSearchForm').addEventListener('submit',async event=>{
      ...
      status.textContent='Searching the anime universe…';results.innerHTML='';
      try{
        const media=await searchMedia(query);
        status.textContent=`${media.length} result${media.length===1?'':'s'} found`;
        results.innerHTML=media.map(item=>{
          ...
          return `<article class="add-result-card" data-media-id="${item.id}"><div class="add-result-cover" style="${cover?`background-image:url('${cover.replace(/'/g,'%27')}')`:''}"><span>${movie?'MOVIE':String(item.format||'SERIES').replaceAll('_',' ')}</span></div><div><h3>${title.replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))}</h3><p>${year} • ${movie?'Movie':`${item.episodes||'?'} episodes`}</p><div class="add-result-actions"><button data-add-mode="planned">ADD TO NOT STARTED</button><button class="primary" data-add-mode="watching">${movie?'MARK WATCHED':'START EP 1'}</button></div></div></article>`;
        }).join('')||'<p class="add-empty">No results found. Try another spelling.</p>';
      }catch(error){status.textContent=error.message||'Search could not connect.'}
    });
    dialog.querySelector('#addAnimeResults').addEventListener('click',async event=>{
      ...
      try{addMedia(await mediaById(card.dataset.mediaId),button.dataset.addMode==='watching')}catch(error){button.disabled=false;button.textContent='TRY AGAIN';dialog.querySelector('#addAnimeStatus').textContent=error.message}
    });
  }

  function openAddDialog(){
    const dialog=document.getElementById('addAnimeDialog');
    if(!dialog)return;
  ...
  function enhanceCards(){
    ...
    document.querySelectorAll('.anime-card').forEach(card=>{
      ...
      const meta=card.querySelector('.card-body .card-meta');
      if(meta)meta.textContent=`${mediaKind(item)} • ${item.status==='completed'?'Watched':'Not watched'}`;
      const action=card.querySelector('[data-action="plus-episode"]');
      if(action&&item.status==='planned')action.textContent='MARK WATCHED';
    });
  }

  function enhanceDialog(){
    const body=document.querySelector('#animeDialog .dialog-body');
    if(!body)return;
    ...
    body.querySelectorAll('.progress-box').forEach((box,index)=>{if(index<3)box.hidden=true});
    const labels=body.querySelectorAll('.progress-box label');if(labels[3])labels[3].textContent='Franchise viewing order';
    const plus=body.querySelector('[data-dialog-action="plus"]');if(plus)plus.hidden=true;
    const finish=body.querySelector('[data-dialog-action="finish-season"]');if(finish)finish.hidden=true;
    const save=body.querySelector('[data-dialog-action="save"]');if(save)save.textContent='Save movie order';
    const complete=body.querySelector('[data-dialog-action="complete"]');if(complete)complete.textContent=`Mark ${mediaKind(item).toLowerCase()} watched`;
  }
  ...
  function interceptExternalAdds(){
    document.addEventListener('click',async event=>{
      const button=event.target.closest('.external-card [data-action="add-planned"],.external-card [data-action="add-watching"]');
      if(!button)return;
      const card=button.closest('[data-external-id]');if(!card)return;
      event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();
      button.disabled=true;button.textContent='ADDING…';
      try{addMedia(await mediaById(card.dataset.externalId),button.dataset.action==='add-watching')}catch(error){button.disabled=false;button.textContent='TRY AGAIN';alert(error.message||'Could not add that title.')}
    },true);
  }

  function install(){
    if(document.documentElement.dataset.libraryManagerV14)return true;
    if(!document.getElementById('homeView'))return false;
    document.documentElement.dataset.libraryManagerV14='1';
    if(!document.querySelector('link[href*="library-manager-v14.css"]')){const link=document.createElement('link');link.rel='stylesheet';link.href='./library-manager-v14.css?release=14';document.head.appendChild(link)}
    const changed=migrateLibrary();
    if(changed&&!sessionStorage.getItem('anime-haven-migrated-v14')){
      sessionStorage.setItem('anime-haven-migrated-v14','1');location.reload();return true;
    }
    sessionStorage.removeItem('anime-haven-migrated-v14');
    createModal();injectAddButtons();enhanceCards();enhanceDialog();showFlash();interceptExternalAdds();
    const observer=new MutationObserver(()=>{injectAddButtons();enhanceCards();enhanceDialog()});
